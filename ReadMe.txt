...
このまま CPS で CPS マシンを作った方がいい気がしてきた。
(2016年  4月 26日 火曜日 11:13:10 JST)

自分の CPS の勉強用に作る。
コンパイラの生成過程をわかりやすく表示できるようにした。
...
(:+ 3 v) とかできない
(:+ 3 5) もできない
  => adhoc対応(vm-codegen と vmgen に散逸している。)
    vm-codegen では :+ :* だけ movei に変えた
    vmgen では 3 v のパターンはは順序を変えた(v 3 になるように swap)
    vmgen では 幾つかの演算子を movei に変えた
----------------------------------------------------------------
...
途中で混乱している。=> ほぼ整理した？
----------------------------------------------------------------
2016年  4月 26日 火曜日 11:19:58 JST
進捗
    cps-compiler として統合して vmgen で作った VM の上で動いた。
    test 中。

    グローバル変数に対応した。
    グローバル関数を fixh でごまかしているので今後ライブラリを作るには
    ちゃんとした対応が必要。
    ...
    spill にバグがあるようだが CPS マシンにする予定なので無視。
        stack/pop のところ。無駄に stack して連続的な record-ref により
