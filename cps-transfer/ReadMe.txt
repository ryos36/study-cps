...
=======================================================
ISSUES
    + apply -> なんでも通ってしまうので fix で定義されていなければ
         エラーにした方がよい。
    + 元の資料では app だがわかりやすいように apply に変更すべき？

=======================================================
FIXED
    + copy-tree をはずすと動きが変わる
        -> ` のなかに既存の @args 等を入れていると copy の必要がある。
           `(,a) ならその必要はないはず(未検証)

    + もとの func を app に変えたものと
        その過程で発生したものの区別がつかない
        HLS だと問題(アセンブラだと jump 命令になるから問題なし)
        -> define は FIXH になる
        もとの FIX も FIXH になる(lambda で利用される)
        いずれも関数だからよい。

=======================================================
let で変換すべき変数が入れ子になっていて変換すべきでないとき
(let ((a 3)....)
   (fix ((f0 (a b c) (+ a b)....
      (+ a 4)

let で a が宣言されているが f0 ないのローカル変数で a が宣言されていて
(+ a b) の a は置き換えてはならない
しかし fix の本体(宣言の後に続くexpr) の a は置き換えられるべき
つまり
   (fix ((f0 (a b c) (+ a b)....
      (+ 3 4)
となる。
