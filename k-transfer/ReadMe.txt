...
fix の上からみた fix 全体の自由変数は
    f0 の自由変数と f1 の自由変数とから f0 f1 を取り除いたものと
    fix area の自由変数
    相互呼び出ししていた場合には複雑になるので注意。
    を合わせたものになる。
    これは基本的には FIX AREA の自由変数が上位の自由変数になるが、
    最終的には FIX AREA から f0 と f1 の(おそらく)すべての関数が
    間接的にでも呼ばれるであろう。クロージャ変換では各関数の
    自由変数を用意するのは FIX AREA だから FIX AREA で結局
    f0 f1 内の自由変数を用意してやらなければならない。
    だたし、fix の上から見ると f0 f1 は FIX AREA 内で既知であるため
    自由変数とはならない。(複雑だ)

    結果として相互呼び出しがあった場合は heap 内に f0 f1 などの関数が
    入るが、相互呼び出しがない場合は heap 内に関数は入らない
    (FIXS では相互呼び出しはないので考慮しない)
    fix の上位への自由変数は常に f0 f1 の関数名は入らない。
